...
const toUpperCase = () => {
  let upperCaseArr = defaultarrvalue.map(item => item.toUpperCase());
  resultContainer.textContent = upperCaseArr.toString();
};
...
const startsWithB = () => {
  let filteredArr = defaultarrvalue.filter(item => item.charAt(0).toUpperCase() === 'B');
...
const findIndex = () => {
  ...
  let index = defaultarrvalue.findIndex(item => item.toLowerCase() === searchValue);
